...
ftdetect.ignoresuffixes = {"~+$", "%.orig$", "%.bak$", "%.old$", "%.new$"}
...
ftdetect.filetypes = {
    ...
    gnuplot = {ext = {"%.dem$", "%.plt$"}},
    go = {ext = {"%.go$"}},
    groovy = {ext = {"%.groovy$", "%.gvy$", "^Jenkinsfile$"}},
    gtkrc = {ext = {"%.gtkrc$"}},
    hare = {ext = {"%.ha$"}},
    ...
    pico8 = {ext = {"%.p8$"}},
    pike = {ext = {"%.pike$", "%.pmod$"}},
    pkgbuild = {ext = {"^PKGBUILD$", "%.PKGBUILD$"}},
    pony = {ext = {"%.pony$"}},
    powershell = {ext = {"%.ps1$"}},
    ...
    spin = {ext = {"%.spin$"}},
    sql = {ext = {"%.ddl$", "%.sql$"}},
    strace = {detect = function(_, data) return data:match("^execve%(") end},
    systemd = {
        ext = {
    ...
    tcl = {utility = {"^tclsh$", "^jimsh$"}, ext = {"%.tcl$", "%.tk$"}},
    texinfo = {ext = {"%.texi$"}},
    text = {
        ext = {"%.txt$"}
...
ftdetect.lookup_lexer = function(filename)
    ...
    if sanitizedfn ~= nil then
        for lang, ft in pairs(ftdetect.filetypes) do
            for _, pattern in pairs(ft.ext or {}) do
                if sanitizedfn:match(pattern) then return lang end
            end
        end
    end
    ...
    if filename ~= nil then
        local file = io.popen(string.format("file -bL --mime-type -- '%s'",
                                            filename:gsub("'", "'\\''")))
        if file then
            mime = file:read('*all')
            file:close()
            if mime then mime = mime:gsub('%s*$', '') end
	...
	--]]
        ...
        if utility or fullhb then
            for lang, ft in pairs(ftdetect.filetypes) do
                if utility and searcher(ft.utility, utility) or fullhb and
                    searcher(ft.hashbang, fullhb) then
                    return lang
                end
            end
        end
