...
-- Builtins.
lex:add_rule('builtin', lex:tag(lexer.FUNCTION_BUILTIN,
                                lex:word_match(lexer.FUNCTION_BUILTIN)))

-- Variable assignment.
local assign = lex:tag(lexer.VARIABLE, lexer.word) *
...
local heredoc = '<<' * P(function(input, index)
    local _, e, minus, _, delimiter = input:find(
                                          '^(%-?)%s*(["\']?)([%w_]+)%2[^\n]*[\n\r\f;]+',
                                          index)
...
local ex_str = -B('\\') * '`'
lex:add_rule('string', lex:tag(lexer.STRING, sq_str + dq_str + heredoc) +
                 lex:tag(lexer.EMBEDDED, ex_str))
...
-- Variables.
local builtin_var = lex:tag(lexer.OPERATOR, '$') *
                        lex:tag(lexer.VARIABLE_BUILTIN,
...
local conditional_op = (num_op + file_op + shell_op + var_op + string_op) *
                           #lexer.space * in_cond_expr
...
lex:set_word_list(lexer.FUNCTION_BUILTIN, {
    ...
    'umask', 'unset', -- Bash built-ins.
    'alias', 'bind', 'builtin', 'caller', 'command', 'declare', 'echo',
    'enable', 'help', 'let', 'local', 'logout', 'mapfile', 'printf', 'read',
    'readarray', 'source', 'type', 'typeset', 'ulimit', 'unalias', --
...
lex:set_word_list(lexer.VARIABLE_BUILTIN, {
    ...
    'BASH_VERSION', 'BASH_XTRACEFD', 'CHILD_MAX', 'COLUMNS', 'COMP_CWORD',
    'COMP_LINE', 'COMP_POINT', 'COMP_TYPE', 'COMP_KEY', 'COMP_WORDBREAKS',
    'COMP_WORDS', 'COMP_REPLY', 'COPROC', 'DIRSTACK', 'EMACS', 'ENV',
    'EPOCHREALTIME', 'EPOCHSECONDS', 'EUID', 'EXECIGNORE', 'FCEDIT', 'FIGNORE',
    'FUNCNAME', 'FUNCNEST', 'GLOBIGNORE', 'GROUPS', 'histchars', 'HISTCMD',
    'HISTCONTROL', 'HISTFILE', 'HISTFILESIZE', 'HISTIGNORE', 'HISTSIZE',
    'HISTTIMEFORMAT', 'HOSTFILE', 'HOSTNAME', 'HOSTTYPE', 'IGNOREEOF',
    'INPUTRC', 'INSIDE_EMACS', 'LANG', 'LC_ALL', 'LC_COLLATE', 'LC_CTYPE',
    'LC_MESSAGES', 'LC_NUMERIC', 'LC_TIME', 'LINENO', 'LINES', 'MACHTYPE',
    'MAILCHECK', 'MAPFILE', 'OLDPWD', 'OPTERR', 'OSTYPE', 'PIPESTATUS',
    'POSIXLY_CORRECT', 'PPID', 'PROMPT_COMMAND', 'PROMPT_DIRTRIM', 'PSO', 'PS3',
    'PS4', 'PWD', 'RANDOM', 'READLINE_LINE', 'READLINE_MARK', 'READLINE_POINT',
    'REPLY', 'SECONDS', 'SHELL', 'SHELLOPTS', 'SHLVL', 'SRANDOM', 'TIMEFORMAT',
    'TMOUT', 'TMPDIR', 'UID', -- Job control.
    'auto_resume'
})
