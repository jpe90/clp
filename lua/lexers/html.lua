...
-- Attributes.
local known_attribute = lex:tag(lexer.ATTRIBUTE,
                                lex:word_match(lexer.ATTRIBUTE, true) +
                                    ((P('data-') + 'aria-') *
                                        (lexer.alnum + '-') ^ 1))
local unknown_attribute = lex:tag(lexer.ATTRIBUTE .. '.unknown',
                                  (lexer.alnum + '-') ^ 1)
...
-- Strings.
local string = lex:tag(lexer.STRING, lexer.after_set('=', lexer.range("'") +
                                                         lexer.range('"')))
...
css_start_rule =
    #(P('style') * lexer.space ^ 0 * '=') * attribute_eq * ws ^ -1 *
...
local js_start_rule = #('<' * script_tag * ('>' + P(
                          function(input, index)
...
local cs_start_rule = #('<' * script_tag * P(
                          function(input, index)
...
lex:set_word_list(lexer.ATTRIBUTE, {
    ...
    'controls', 'coords', 'data', 'data-', 'datetime', 'default', 'defer',
    'dir', 'dirname', 'disabled', 'download', 'draggable', 'dropzone',
    'enctype', 'for', 'form', 'headers', 'height', 'hidden', 'high', 'href',
    'hreflang', 'http-equiv', 'icon', 'id', 'ismap', 'itemprop', 'keytype',
    'kind', 'label', 'lang', 'language', 'list', 'loop', 'low', 'manifest',
    'max', 'maxlength', 'media', 'method', 'min', 'multiple', 'name',
    'novalidate', 'open', 'optimum', 'pattern', 'ping', 'placeholder', 'poster',
    'preload', 'pubdate', 'radiogroup', 'readonly', 'rel', 'required',
    'reversed', 'role', 'rows', 'rowspan', 'sandbox', 'scope', 'scoped',
    'seamless', 'selected', 'shape', 'size', 'sizes', 'span', 'spellcheck',
    'src', 'srcdoc', 'srclang', 'start', 'step', 'style', 'summary', 'tabindex',
