...
local stl_type = lex:tag(lexer.TYPE .. '.stl',
                         'std::' * lex:word_match(lexer.TYPE .. '.stl'))
lex:add_rule('type', basic_type + stl_type * -P('::'))

-- Functions.
local non_member = -(B('.') + B('->') + B('::'))
local builtin_func = lex:tag(lexer.FUNCTION_BUILTIN, P('std::') ^ -1 *
                                 lex:word_match(lexer.FUNCTION_BUILTIN))
local stl_func = lex:tag(lexer.FUNCTION_BUILTIN .. '.stl', 'std::' *
                             lex:word_match(lexer.FUNCTION_BUILTIN .. '.stl'))
local func = lex:tag(lexer.FUNCTION, lexer.word)
local method = (B('.') + B('->')) * lex:tag(lexer.FUNCTION_METHOD, lexer.word)
lex:add_rule('function',
             (non_member * (stl_func + builtin_func) + method + func) *
                 #(lexer.space ^ 0 * '('))

-- Constants.
local const = lex:tag(lexer.CONSTANT_BUILTIN,
                      P('std::') ^ -1 * lex:word_match(lexer.CONSTANT_BUILTIN))
local stl_const = lex:tag(lexer.CONSTANT_BUILTIN .. '.stl', 'std::' *
                              lex:word_match(lexer.CONSTANT_BUILTIN .. '.stl'))
...
lex:add_rule('string',
             lex:tag(lexer.STRING, ('u8' + S('LuU')) ^ -1 * (sq_str + dq_str)))
...
local include = lex:tag(lexer.PREPROCESSOR, '#' * S('\t ') ^ 0 * 'include') *
                    (lex:get_rule('whitespace') *
                        lex:tag(lexer.STRING, lexer.range('<', '>', true))) ^ -1
local preproc = lex:tag(lexer.PREPROCESSOR,
...
lex:set_word_list(lexer.KEYWORD, {
    ...
    'typename', 'using', 'virtual', 'volatile', 'while', -- Operators.
    'and', 'and_eq', 'bitand', 'bitor', 'compl', 'not', 'not_eq', 'or', 'or_eq',
    'xor', 'xor_eq', -- C++11.
    'alignas', 'alignof', 'constexpr', 'decltype', 'final', 'noexcept',
    'nullptr', 'override', 'static_assert', 'thread_local', --
...
lex:set_word_list(lexer.TYPE, {
    ...
    'byte', -- C++17
    -- <cstdint>
    'int8_t', 'int16_t', 'int32_t', 'int64_t', 'int_fast8_t', 'int_fast16_t',
...
lex:set_word_list(lexer.TYPE .. '.stl', {
    ...
    'floating_point', 'assignable_from', 'swappable', 'swappable_with',
    'destructible', 'constructible_from', 'default_initializable',
    'move_constructible', 'copy_constructible', 'equality_comparable',
    'equality_comparable_with', 'movable', 'copyable', 'semiregular', 'regular',
    'invocable', 'regular_invocable', 'predicate', 'relation',
    ...
    -- <future> C++11
    'promise', 'packaged_task', 'future', 'shared_future', 'launch',
    'future_status', 'future_error', 'future_errc', --
    'initializer_list', -- <initializer_list>
    'istream', 'iostream', -- <istream>
    -- <iterator>
    'reverse_iterator', 'back_insert_iterator', 'front_insert_iterator',
    'insert_iterator', 'istream_iterator', 'ostream_iterator',
    ...
    'fisher_f_distribution', 'student_t_distribution', 'discrete_distibution',
    'piecewise_constant_distribution', 'piecewise_linear_distribution',
    'seed_seq', --
    'ratio', -- <ratio> C++11
    -- <regex> C++11
    'regex', 'csub_match', 'ssub_match', 'cmatch', 'smatch', 'cregex_iterator',
    'sregex_iterator', 'cregex_token_iterator', 'sregex_token_iterator',
    ...
    'stack', -- <stack>
    -- <stdexcept>
    'logic_error', 'invalid_argument', 'domain_error', 'length_error',
    ...
    'pair', -- <utility>
    'variant', 'monostate', 'bad_variant_access', 'variant_size',
    'variant_alternative', -- <variant> C++17
    'vector' -- <vector>
})
...
lex:set_word_list(lexer.FUNCTION_BUILTIN, {
    ...
    -- <cmath>
    'abs', 'fmod', 'exp', 'log', 'log10', 'pow', 'sqrt', 'sin', 'cos', 'tan',
    'asin', 'acos', 'atan', 'atan2', 'sinh', 'cosh', 'tanh', 'ceil', 'floor',
    'frexp', 'ldexp', 'modf', -- C++11.
    'remainder', 'remquo', 'exp2', 'expm1', 'log2', 'log1p', 'cbrt', 'hypot',
    'asinh', 'acosh', 'atanh', 'erf', 'erfc', 'tgamma', 'lgamma', 'trunc',
    'round', 'nearbyint', 'rint', 'scalbn', 'ilogb', 'logb', 'nextafter',
    'copysign', 'isfinite', 'isinf', 'isnan', 'isnormal', 'signbit',
    'isgreater', 'isgreaterequal', 'isless', 'islessequal', 'islessgreater',
    'isunordered', --
    -- C++17.
    'assoc_laguerre', 'assoc_legendre', 'beta', 'comp_ellint_1',
    'comp_ellint_2', 'comp_ellint_3', 'cyl_bessel_i', 'cyl_bessel_j',
    'cyl_bessel_k', 'cyl_neumann', 'ellint_1', 'ellint_2', 'ellint_3', 'expint',
    'lhermite', 'lgendre', 'laguerre', 'riemann_zeta', 'sph_bessel',
    'sph_legendre', 'sph_neumann', --
    'lerp', -- C++20
    -- <cstring>
    'strcpy', 'strncpy', 'strcat', 'strncat', 'strxfrm', 'strlen', 'strcmp',
    'strncmp', 'strcoll', 'strchr', 'strrchr', 'strspn', 'strcspn', 'strpbrk',
    'strstr', 'strtok', 'memchr', 'memcmp', 'memset', 'memcpy', 'memmove',
    'strerror'
})

lex:set_word_list(lexer.FUNCTION_BUILTIN .. '.stl', {
    -- <algorithm>
    'for_each', 'count', 'count_if', 'mismatch', 'find', 'find_if', 'find_end',
    'find_first_of', 'adjacent_find', 'search', 'search_n', 'copy',
    'copy_backward', 'fill', 'fill_n', 'transform', 'generate', 'generate_n',
    'remove', 'remove_if', 'remove_copy', 'remove_copy_if', 'replace',
    'replace_if', 'replace_copy', 'replace_copy_if', 'swap', 'swap_ranges',
    'iter_swap', 'reverse', 'reverse_copy', 'rotate', 'rotate_copy',
    'unique_copy', 'partition', 'stable_partition', 'sort', 'partial_sort',
    'partial_sort_copy', 'stable_sort', 'nth_element', 'lower_bound',
    'upper_bound', 'binary_search', 'equal_range', 'merge', 'inplace_merge',
    'includes', 'set_difference', 'set_intersection',
    'set_symmetric_difference', 'set_union', 'make_heap', 'push_heap',
    'pop_heap', 'sort_heap', 'max', 'max_element', 'min', 'min_element',
    'equal', 'lexicographical_compare', 'next_permutation', 'prev_permutation', --
    -- C++11.
    'all_of', 'any_of', 'none_of', 'find_if_not', 'copy_if', 'copy_n', 'move',
    'move_backward', 'shuffle', 'is_partitioned', 'partition_copy',
    'partition_point', 'is_sorted', 'is_sorted_until', 'is_heap',
    'is_heap_until', 'minmax', 'minmax_element', 'is_permutation', --
    ...
    -- <bit> C++20
    'bit_cast', 'byteswap', 'has_single_bit', 'bit_ceil', 'bit_floor',
    'bit_width', 'rotl', 'rotr', 'countl_zero', 'countl_one', 'countl_zero',
    'countr_one', 'popcount', --
    'from_chars', 'to_chars', -- <charconv> C++17
    -- <format> C++20
    'format', 'format_to', 'format_to_n', 'formatted_size', 'vformat',
    'vformat_to', 'visit_format_arg', 'make_format_args', --
    ...
    -- <memory>
    'make_shared', 'allocate_shared', 'static_pointer_cast',
    ...
    'iota', -- C++11
    'reduce', 'transform_reduce', 'inclusive_scan', 'exclusive_scan', 'gcd',
    'lcm', -- C++17
    'midpoint', -- C++20
...
lex:set_word_list(lexer.PREPROCESSOR, {
    ...
    'ifndef', 'import', 'line', 'pragma', 'undef', 'using', 'warning', --
