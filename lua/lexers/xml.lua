...
local doctype = lex:tag(lexer.TAG .. '.doctype', '<!DOCTYPE') * ws *
                    lex:tag(lexer.TAG .. '.doctype', identifier) *
                    (ws * identifier) ^ -1 * (1 - P('>')) ^ 0 *
...
-- Tags.
local namespace = lex:tag(lexer.OPERATOR, ':') *
                      lex:tag(lexer.LABEL, identifier)
lex:add_rule('element', lex:tag(lexer.TAG, '<' * P('/') ^ -1 * identifier) *
                 namespace ^ -1)
...
-- Attributes.
local attribute_eq =
    lex:tag(lexer.ATTRIBUTE, identifier) * namespace ^ -1 * ws ^ -1 * equals
...
lex:add_rule('string',
             lex:tag(lexer.STRING, lexer.after_set('=', sq_str + dq_str)))
