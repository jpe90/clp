...
lex:add_rule('type', token(lexer.TYPE, word_match {
    'void', 'bool', 'char', 'byte', 'int', 'uint', 'int8', 'uint8', 'int16',
    'uint16', 'int32', 'uint32', 'int64', 'uint64', 'flt32', 'flt64'
} + '@' * lexer.word))
...
local integer = '0x' * xdigit ^ 1 + '0o' * odigit ^ 1 + '0b' * bdigit ^ 1 +
                    digit ^ 1
local float = digit ^ 1 *
                  ((('.' * digit ^ 1) * (S('eE') * S('+-') ^ -1 * digit ^ 1) ^
                      -1) +
                      (('.' * digit ^ 1) ^ -1 * S('eE') * S('+-') ^ -1 * digit ^
                          1))
