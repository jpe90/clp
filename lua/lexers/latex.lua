...
local math_begin_end = (P('begin') + P('end')) * P(' ') ^ 0 * '{' * math_word *
                           P('*') ^ -1 * '}'
lex:add_rule('math', lex:tag('environment.math',
                             '$' + '\\' * (S('[]()') + math_begin_end)))

-- LaTeX environments.
lex:add_rule('environment',
             lex:tag('environment',
                     '\\' * (P('begin') + 'end') * P(' ') ^ 0 * '{' * lexer.word *
                         P('*') ^ -1 * '}'))

-- Sections.
lex:add_rule('section', lex:tag('command.section', '\\' * word_match(
