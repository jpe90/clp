...
lex:add_rule('internal_link',
             B('[[') * token(lexer.LINK, (lexer.any - '|' - ']]') ^ 1))

-- Templates and parser functions.
lex:add_rule('template', token(lexer.OPERATOR, S('{}')))
lex:add_rule('parser_func', B('{{') *
                 token(lexer.FUNCTION,
                       '#' * lexer.alpha ^ 1 + lexer.upper ^ 1 * ':'))
lex:add_rule('template_name',
             B('{{') * token(lexer.LINK, (lexer.any - S('{}|')) ^ 1))
...
lex:add_rule('behavior_switch',
             (B(lexer.space) + start_pat) * token('behavior_switch', word_match(
