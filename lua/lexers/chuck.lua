...
lex:add_rule('function',
             lex:tag(lexer.FUNCTION_BUILTIN, std + machine + math) + func)

-- Constants.
lex:add_rule('constant', lex:tag(lexer.CONSTANT_BUILTIN, 'Math.' *
                                     lex:word_match(
                                         lexer.CONSTANT_BUILTIN .. '.math')))

-- Global ugens.
lex:add_rule('ugen', lex:tag(lexer.CONSTANT_BUILTIN .. '.ugen',
                             word_match('dac adc blackhole')))

-- Times.
lex:add_rule('time', lex:tag(lexer.NUMBER,
                             word_match('samp ms second minute hour day week')))
...
lex:set_word_list(lexer.KEYWORD, {
    ...
    'until', 'while', -- Other chuck keywords.
...
lex:set_word_list(lexer.CLASS, {
    ...
    'public', 'pure', 'static', 'super', 'this'
})

lex:set_word_list(lexer.FUNCTION_BUILTIN, {
    'abs', 'fabs', 'sgn', 'system', 'atoi', 'atof', 'getenv', 'setenv', 'mtof',
    'ftom', 'powtodb', 'rmstodb', 'dbtopow', 'dbtorms'
})
...
lex:set_word_list(lexer.FUNCTION_BUILTIN .. '.math', {
    ...
    'tanh', 'hypot', 'pow', 'sqrt', 'exp', 'log', 'log2', 'log10', 'random',
    'random2', 'randomf', 'random2f', 'srandom', 'floor', 'ceil', 'round',
    'trunc', 'fmod', 'remainder', 'min', 'max', 'nextpow2', 'isinf', 'isnan'
})
