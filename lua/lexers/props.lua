...
-- Identifiers.
lex:add_rule('identifier', lex:tag(lexer.IDENTIFIER, (lexer.alpha + S('.-_')) *
                                       (lexer.alnum + S('.-_') ^ 0)))

-- Colors.
local xdigit = lexer.xdigit
lex:add_rule('color', lex:tag(lexer.NUMBER, '#' * xdigit * xdigit * xdigit *
...
lex:add_rule('variable', lex:tag(lexer.OPERATOR, '$(') *
                 lex:tag(lexer.VARIABLE,
                         (lexer.nonnewline - lexer.space - ')') ^ 0) *
