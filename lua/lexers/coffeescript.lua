...
lex:add_rule('keyword', lex:tag(lexer.KEYWORD, word_match {
    ...
    'instanceof', 'is', 'isnt', 'let', 'loop', 'native', 'new', 'no', 'not',
    'of', 'off', 'on', 'or', 'return', 'super', 'switch', 'then', 'this',
    'throw', 'true', 'try', 'typeof', 'unless', 'until', 'var', 'void', 'when',
    'while', 'with', 'yes'
}))

-- Fields: object properties and methods.
lex:add_rule('field', lex:tag(lexer.FUNCTION, '.' * (S('_$') + lexer.alpha) *
                                  (S('_$') + lexer.alnum) ^ 0))
