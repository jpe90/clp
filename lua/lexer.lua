...
local predefined = {
    'default', 'line.number', 'brace.light', 'brace.bad', 'control.char',
    'indent.guide', 'call.tip', 'fold.display.text'
}
...
function M.word_match(lexer, word_list, case_insensitive)
    if type(lexer) == 'table' and getmetatable(lexer) then
        if lexer._lexer then
            ...
            if not parent._CHILDREN or not parent._CHILDREN[lexer] then
                lexer = parent
            end
        end

        if not lexer._WORDLISTS then
            lexer._WORDLISTS = {case_insensitive = {}}
        end
    ...
    for _, word in ipairs(word_list) do
        ...
        for char in word:gmatch('[^%w_%s]') do
            if not extra_chars:find(char, 1, true) then
                extra_chars = extra_chars .. char
            end
        end
    end
        ...
        for tag_name, symbols in pairs(child._fold_points) do
            if tag_name ~= '_symbols' then
                for symbol, v in pairs(symbols) do
                    lexer:add_fold_point(tag_name, symbol, v)
                end
            end
        end
    end
...
function M.add_fold_point(lexer, tag_name, start_symbol, end_symbol)
    ...
    -- parent lexer.
    if lexer._lexer then
        lexer._lexer:add_fold_point(tag_name, start_symbol, end_symbol)
    end
end
...
local function add_lexer(g, lexer)
    ...
    if lexer._WORDLISTS then
        for i = 1, #lexer._WORDLISTS do
            ...
            local list, case_insensitive = lexer._WORDLISTS[i],
                                           lexer._WORDLISTS.case_insensitive[i]
            local patt = list ~= '' and M.word_match(list, case_insensitive) or
                             P(false)
    ...
    if lexer._end_rules then
        for parent, end_rule in pairs(lexer._end_rules) do
            ...
            rule =
                rule - V(back_id) + -- (V('css.property') + ... + V('css_fallback')) - V('css_to_html')
...
local function build_grammar(lexer, init_style)
    ...
    if not lexer._initial_rule then
        lexer._initial_rule = lexer._parent_name or lexer._name
    end
    ...
    if lexer._CHILDREN then
        for tag, style_num in pairs(lexer._TAGS) do
            if style_num == init_style then
                local lexer_name = tag:match('^whitespace%.(.+)$') or
                                       lexer._parent_name or lexer._name
    ...
    if lexer._lex_by_line then
        ...
        local function append(tags, line_tags, offset)
            for i = 1, #line_tags, 2 do
                tags[#tags + 1], tags[#tags + 2] = line_tags[i],
                                                   line_tags[i + 1] + offset
            end
        end
        local tags = {}
        local offset = 0
        rawset(M, 'line_from_position',
               function(pos) return line_from_position(pos + offset) end)
...
function M.fold(lexer, text, start_line, start_level)
    ...
    if fold and lexer._fold_points then
        ...
        local fold_zero_sum_lines =
            M.property_int['fold.scintillua.on.zero.sum.lines'] > 0
            ...
            if line ~= '' then
                ...
                local function is_valid_range(s, e)
                    ...
                    for i = 1, #ranges - 1, 2 do
                        ...
                        if s >= range_s and s <= range_e or e >= range_s and e <=
                            range_e then return false end
                    end
                    ranges[#ranges + 1] = s
                    ranges[#ranges + 1] = e
                    return true
                ...
                folds[line_num] = prev_level +
                                      (fold_compact and FOLD_BLANK or 0)
            end
            line_num = line_num + 1
        end
    elseif fold and
        (lexer._fold_by_indentation or
            M.property_int['fold.scintillua.by.indentation'] > 0) then
...
function M.new(name, opts)
    local lexer = setmetatable({
        ...
        _lex_by_line = opts and opts['lex_by_line'],
        _fold_by_indentation = opts and opts['fold_by_indentation'],
        _case_insensitive_fold_points = opts and
            opts['case_insensitive_fold_points'],
        _no_user_word_lists = opts and opts['no_user_word_lists'],
        _lexer = opts and opts['inherit']
    }, {
        __index = {
            tag = M.tag,
            word_match = M.word_match,
            set_word_list = M.set_word_list,
            add_rule = M.add_rule,
            modify_rule = M.modify_rule,
            get_rule = M.get_rule,
            add_fold_point = M.add_fold_point,
            embed = M.embed,
            lex = M.lex,
            fold = M.fold, --
...
--        ['scintillua.lexers'] = package.path:gsub('/lexers?%?%.lua', '')
    }, {
...
function M.load(name, alt_name)
    ...
    local path = M.property['scintillua.lexers']:gsub(';', '/lexers/?.lua;') ..  M.property['scintillua.lexers']:gsub(';', '/?.lua;') ..
                     '/?.lua' .. '/lexers/?.lua'
    ...
    M.property['scintillua.comment.' .. (alt_name or name)] =
        M.property['scintillua.comment']
...
function M.detect(filename, line)
    if not filename then
        filename = M.property and M.property['lexer.scintillua.filename'] or ''
    end
    if not line then
        line = M.property and M.property['lexer.scintillua.line'] or ''
    end
    ...
    for patt, name in pairs(M.detect_patterns) do
        if line:find(patt) then return name end
    end
    for patt, name in pairs(patterns) do
        if line:find(patt) then return name end
    end
    local name, ext = filename:match('[^/\\]+$'), filename:match('[^.]*$')
    return M.detect_extensions[name] or extensions[name] or
               M.detect_extensions[ext] or extensions[ext]
end
...
-- @name bin_num_
function M.bin_num_(c)
    return '0' * S('bB') * (P(c) ^ -1 * S('01')) ^ 1 * -M.xdigit
end
---
...
function M.integer_(c)
    return S('+-') ^ -1 *
               (M.hex_num_(c) + M.bin_num_(c) + M.oct_num_(c) + M.dec_num_(c))
end
local function exp_(c)
    return S('eE') * S('+-') ^ -1 * M.digit * (P(c) ^ -1 * M.digit) ^ 0
end
---
...
function M.float_(c)
    return S('+-') ^ -1 *
               ((M.dec_num_(c) ^ -1 * '.' * M.dec_num_(c) + M.dec_num_(c) * '.' *
                   M.dec_num_(c) ^ -1 * -P('.')) * exp_(c) ^ -1 +
                   (M.dec_num_(c) * exp_(c)))
end
---
...
function M.to_eol(prefix, escape)
    return (prefix or M.nonnewline) *
               (not escape and M.nonnewline or 1 - (M.newline + '\\') + '\\' *
                   M.any) ^ 0
end
...
function M.after_set(set, patt, skip)
    ...
    return (B(S(set)) + -B(1)) * patt +
               Cmt(C(patt), function(input, index, match, ...)
            local pos = index - #match
            if #skip > 0 then
                while pos > 1 and skip_chars[input:byte(pos - 1)] do
                    pos = pos - 1
                end
            end
            if pos == 1 or set_chars[input:byte(pos - 1)] then
                return index, ...
            end
            return nil
        end)
...
M.styles = setmetatable({}, { -- legacy
    __index = function()
        return setmetatable({}, {__concat = function() return nil end})
    end
})
